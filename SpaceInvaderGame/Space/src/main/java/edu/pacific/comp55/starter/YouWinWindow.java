...
public class YouWinWindow extends JFrame {
    ...
    public YouWinWindow() {
        initializeUI();
    }public YouWinWindow(Board boardInst, boolean mode) {
    ...
    private void initializeUI() {
        ...
        JLabel loseLabel = new JLabel("YOU WIN");
        loseLabel.setBounds(110, 50, 300, 50);
        loseLabel.setFont(customFont.deriveFont(30.0f));
        loseLabel.setForeground(Color.GREEN);
        add(loseLabel);

        retryLabel = createLabel("Restart", 115, 150);
    ...
    private static Font loadCustomFont(String fontFileName) {
        try {
            URL fontUrl = YouWinWindow.class.getClassLoader().getResource(fontFileName);
